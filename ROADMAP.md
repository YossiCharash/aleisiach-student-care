# ROADMAP — מערכת ניהול חניךים (עלי שיח)
...
- [ ] מספר תקופת שמירה (retention) לחניך מאורכב
- [ ] בחירת גרסת עיצוב לכניסה / מסך חניך
...
- [x] **סינון הרשאות על students**: מדריך→סדנתו · מורה מקצועי→read-only · `archived_by` למשתמש הנוכחי
- [x] ניהול סדנאות (Class) — CRUD מלא: רשימה (כל משתמש מאומת) + יצירה/שינוי-שם (מנהל) + audit
...
- [x] Tab 3 — סיכום עו"ס (מנהל כותב, מדריך קורא, מורה מקצועי חסום)
- [x] Tab 4 — פרטי חניך: זהות · אבחנות · אנשי קשר · אפוטרופסות+מעמד (RBAC דו-אזורי, גיל מחושב)
...
- [x] מסך ראשי: שם עובד + רשימת חניךים (לפי הרשאה, נאכף בשרת)
- [x] מסך חניך — טאב 1 (תוכנית, נגזר לקריאה בלבד)
- [x] מסך חניך — טאב 2 (ישיבות צוות, אקורדיון דינמי מהטקסונומיה + ייצוא PDF מאומת)
- [x] מסך חניך — טאב 3 (סיכום עו"ס, כתיבה למנהל בלבד)
- [x] מסך חניך — טאב 4 (פרטי חניך, צפייה+עריכה, הסתרת מידע רגיש למורה מקצועי)
- [x] תצוגת "חניךים בארכיון" (מנהל בלבד, מסלול `/students/archived` מאחורי `ManagerRoute` + קישור בהדר) — רשימה מ-`GET /students/archived` עם כפתור שחזור ל-`POST /students/{id}/restore`
- [x] עמוד הגדרות (מנהל): אזור משתמשים (הזמנה/השבתה/הפעלה) + אזור סדנאות (יצירה/שינוי-שם) + ניהול טקסונומיה (הוספה + עריכת שם + השבתה + **הפעלה מחדש בכל הרמות**: תוויות · תת-תוויות · כישורים · פתרונות)
...
- [x] בדיקות: Vitest + RTL + Playwright E2E — עשן ציבורי (כניסה/ניתוב/שכחתי-סיסמה/404) **וגם זרימות מאומתות מאחורי כניסה** (מנהל/מדריך/מורה-מקצועי מול seed: רשימת חניךים, 4 טאבים, הגדרות, ארכיון, שינוי-סיסמה, RBAC), ב-CI
- [ ] כותרות טאב 4 בהגדרות — חסום על שמות הכותרות (כמו ב-backend)
- [x] **בורר סדנה** — `WorkshopPicker` נטען מ-`GET /classes`, מחליף את שדה `workshop_id` הידני ב-CreateStudentDialog/InviteUserDialog; `ClassIdField` הוסר. נוסף אזור "סדנאות" בהגדרות (יצירה/שינוי-שם דרך `POST`/`PATCH /classes`)

## 5. איכות ונתונים
- [x] בדיקות יחידה לכל רכיב backend חדש (test-with-code) — 123 טסטים
- [x] נתוני דמו (seed) לכל התפקידים והחניךים — `python -m backend.app.seed` (אידמפוטנטי): 3 תפקידים, 2 סדנאות, 3 חניךים, טקסונומיה מלאה, ישיבה+פרטים+סיכום עו"ס
- [x] בדיקות E2E מקצה-לקצה — Playwright מלא: זרימות ציבוריות + מאומתות מאחורי כניסה. job ייעודי `e2e` ב-CI מריץ מחסנית מלאה (שירות Postgres → `alembic upgrade head` → seed → uvicorn → `pnpm test:e2e`)
- [ ] סקירת אבטחה לפני production

---

### הבא בתור (מומלץ)
1. ~~**Endpoint לרשימת סדנאות** (Class CRUD)~~ — **בוצע** (PR #25).
...
4. ~~**שחזור/רשימת חניךים מאורכבים**~~ — **בוצע**: `GET /students/archived` + `POST /students/{id}/restore` (מנהל). נותר: תצוגת ארכיון ב-frontend.
...
6. ~~**חיווט frontend**: בורר סדנה, שינוי סיסמה, תצוגת ארכיון, הפעלה-מחדש בטקסונומיה~~ — **בוצע** (שלב C, PRs #32/#33/#34 + הפעלה-מחדש).
7. ~~**בדיקות E2E (Playwright)** מאחורי כניסה~~ — **בוצע**: זרימות מאומתות + job `e2e` מלא-מחסנית ב-CI.
