...
- [x] עמוד הגדרות (מנהל): אזור משתמשים (הזמנה/השבתה/הפעלה) + ניהול טקסונומיה
- [~] הגדרות אישיות: "שינוי סיסמה" דרך שליחת קישור איפוס לדוא"ל (חסר endpoint ייעודי מאומת בשרת)
- [~] בדיקות: Vitest + RTL — 24 טסטים (utils/permissions/client/buildEntries/RatingPill). Playwright E2E — טרם
