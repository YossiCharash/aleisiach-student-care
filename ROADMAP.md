# ROADMAP — מערכת ניהול חניכים (עלי שיח)
...
- [x] מסך ראשי: שם עובד + רשימת חניכים (לפי הרשאה, נאכף בשרת)
...
- [x] תצוגת "חניכים בארכיון" (מנהל בלבד, מסלול `/students/archived` מאחורי `ManagerRoute` + קישור בהדר) — רשימה מ-`GET /students/archived` עם כפתור שחזור ל-`POST /students/{id}/restore`
...
- [x] בדיקות: Vitest + RTL + Playwright E2E — עשן ציבורי (כניסה/ניתוב/שכחתי-סיסמה/404) **וגם זרימות מאומתות מאחורי כניסה** (מנהל/מדריך/מורה-מקצועי מול seed: רשימת חניכים, 4 טאבים, הגדרות, ארכיון, שינוי-סיסמה, RBAC), ב-CI
...
- [x] נתוני דמו (seed) לכל התפקידים והחניכים — `python -m backend.app.seed` (אידמפוטנטי): 3 תפקידים, 2 סדנאות, 3 חניכים, טקסונומיה מלאה, ישיבה+פרטים+סיכום עו"ס
...
4. ~~**שחזור/רשימת חניכים מאורכבים**~~ — **בוצע**: `GET /students/archived` + `POST /students/{id}/restore` (מנהל). נותר: תצוגת ארכיון ב-frontend.
