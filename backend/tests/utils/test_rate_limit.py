...
from backend.app.utils.routes.rate_limit import _client_ip
...
def test_uses_peer_address_when_no_proxy_is_trusted() -> None:
    ...
    assert _client_ip(request, trusted_proxy_count=0) == "10.0.0.1"


def test_takes_client_ip_from_forwarded_header_behind_one_proxy() -> None:
...
def test_falls_back_to_unknown_when_client_missing() -> None:
    ...
    assert _client_ip(request, trusted_proxy_count=0) == "unknown"
